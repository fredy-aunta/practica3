...
class Tank_auth
{
	...
	function __construct()
	{
		...
		$this->ci->load->library(array('session','facebook_lib'));
	...
	public function login_with_fb() {
        $accessToken = $this->ci->facebook_lib->get_access_token();
        if(empty($accessToken)){
            $login_url = $this->ci->facebook_lib->get_login_url(site_url('/auth/login_fb'));
            header('Location: ' . $login_url);
            exit();
        }
        $user = $this->ci->facebook_lib->get_user_info();
        return $user;
	}

	public function login_fb_user($cliente) {
		$user = $this->ci->users->get_user_by_id($cliente->user_id, true);
		$this->_store_login($user, $cliente->email, false);
	}

	function _store_login($user, $login){
		$cliente = $this->ci->cliente_dao->get_by_user_id($user->id);
